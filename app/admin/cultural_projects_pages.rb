ActiveAdmin.register CulturalProjectsPage do
  ...
  controller do
    def index
      redirect_to admin_cultural_projects_page_path(CulturalProjectsPage.all.first)
    end
  end
  ...
  show title: :page_label do
    columns do
      column do
        attributes_table do
          row (:heading) { |cultural_project| raw(cultural_project.heading) }
          row (:subheading) { |cultural_project| raw(cultural_project.subheading) }
        end
      end
      column do
  ...
  member_action :delete_image, method: :delete do
    ...
    @img.purge_later
    redirect_back(fallback_location: edit_admin_cultural_projects_page_path)
  end
